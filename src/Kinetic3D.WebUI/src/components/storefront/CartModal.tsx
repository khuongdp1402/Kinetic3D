...
export function CartModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex justify-end">
      <div className="w-full max-w-md h-full bg-[#0B0B0B] border-l border-neutral-800 p-6 flex flex-col text-[#F3F4F6]">
                ...
                items.map((item) => (
                  <div key={item.id} className="bg-neutral-900 rounded p-4 border border-neutral-800 flex flex-col gap-2 relative">
                    <button
                      ...
                      className="absolute top-2 right-2 text-neutral-500 hover:text-[#FF5F1F] transition-colors"
                    >
                      ✕
                    </button>
